...
namespace NN.Configuration
{
    public struct StateDict : IByteSerializable
    {
        ...
        {
            var numParameters = reader.ReadInt();

            selfParameters  = new Dictionary<ModuleParameterName, Tensor>();
            childParameters = new List<(int[] path, (ModuleParameterName name, Tensor value) parameter)>();

            for (var i = 0; i < numParameters; i++) {
                var parameterNameString = reader.ReadString();

                var parameterValue = reader.Read<Tensor>();
        ...
        public override string ToString()
        {
            ...
            var selfBytes = selfParameters.Count > 0
                                    ? selfParameters.Select(kvp => kvp.Key + ":\n" + kvp.Value).Aggregate((a, b) => a + "\n" + b) + "\n"
                                    : "";
