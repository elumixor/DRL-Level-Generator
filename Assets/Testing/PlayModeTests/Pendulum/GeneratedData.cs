...
namespace Testing.PlayModeTests.Pendulum {
    public class GeneratedData : Vector {
        ...
        public int EnemiesCount => Mathf.RoundToInt(values[4]);

        const int ENEMIES_OFFSET = 5;

        public CircleConfiguration GetEnemy(int i) => new CircleConfiguration(
            values[ENEMIES_OFFSET + i * 3],
            values[ENEMIES_OFFSET + i * 3 + 1],
            values[ENEMIES_OFFSET + i * 3 + 2]);

        public GeneratedData(float connectorLength,
            float bobRadius,
            float angle,
