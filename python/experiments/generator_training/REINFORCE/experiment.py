...
def main(context, enemy_x_min, enemy_x_max, epochs, batch_size, sample_size, lr, constrain_weight, min_std,
         std_constrain, convergence, target, constrain, d_in_sampling):
    ...
    if d_in_sampling == "systematic":
        d_in = torch.linspace(0, 1, batch_size).unsqueeze(1)

    # Batch of inputs
    for _ in range(epochs):
        # Generate a random batch of input difficulties
        if d_in_sampling == "random":
