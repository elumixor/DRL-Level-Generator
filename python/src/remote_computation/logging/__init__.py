...
plotters: Dict[int, Logger] = dict()


def show(model_id: int, data: LogData, options: LogOptions):
    # store logger for each model
    if model_id not in plotters:
        logger = Logger(model_id, options)
        plotters[model_id] = logger

    logger = plotters[model_id]
    logger.update(data)
